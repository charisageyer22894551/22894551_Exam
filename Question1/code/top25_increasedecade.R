# # top 25 percent increase per decade
#
...
#         ungroup()
#
#     # Step 3: Factor reorder names by overall total
...
#
#     # Step 4: Plot % increase instead of Total
#    g <- ggplot(Top25_plot, aes(x = name, y = factor(decade), size = perc_change, colour = name)) +
#         geom_point(alpha = 0.7) +
#         scale_size_continuous(range = c(1, 10), name = "% Change") +
#         # coord_flip() +
#         theme_bw() +
#         labs(
#             title = glue::glue("Top 25 {gender_label} Names – % Increase per Decade"),
#             subtitle = "Bubble size shows % increase in name usage from previous decade",
#             x = "Name", y = "Decade"
#         ) +
#         theme(
...
#         ungroup()
#
#     #additional -> make the top 3 stand out
#     Top25_with_change <- Top25_with_change %>%
#         group_by(decade) %>%
#         mutate(bubble_rank = rank(-perc_change)) %>%
#         mutate(colour_group = ifelse(bubble_rank <= 3, "Top 3", name))
#
#     # Step 3: Factor reorder names by overall total
#     name_order <- Top25_with_change %>%
#         group_by(name) %>%
...
#         geom_point(data = filter(Top25_plot, !is.na(highlight_top3)), aes(colour = highlight_top3), size = 6, alpha = 0.9, show.legend = TRUE) +
#         geom_point(alpha = 0.7) +
#         scale_size_continuous(range = c(1, 10), name = "% Change") +
