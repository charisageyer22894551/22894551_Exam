...
assumed_height_cm <- 162  # global average female height

     Health_lpm <- Health %>%
  mutate(healthy_bmr = ifelse(
    `BMR (Calories)` / `Current Weight (lbs)`*2.2 > 25 &
    `BMR (Calories)` / `Current Weight (lbs)`*2.2 < 33,
    1, 0
  )) %>% 
...
# Create categorical variables      
     ...
     Health_lpm %>%
  ggplot(aes(x = `Stress Level`, fill = as.factor(healthy_bmr))) +
  geom_bar(position = "fill") +  # "fill" shows proportions
  labs(title = "Proportion of Healthy BMR by Stress Level",
       x = "Stress Level", y = "Proportion",
       fill = "Healthy BMR") +
  ...
  theme_minimal()
     ...
     Health_lpm %>%
  ggplot(aes(x = `Physical Activity Level`, fill = as.factor(healthy_bmr))) +
  geom_bar(position = "fill") +  # "fill" shows proportions
  labs(title = "Proportion of Healthy BMR by Physical Activity level",
