...
I will work at a national level. I want to have a summary table (bubble plot) at the end with the 5 most popular names for each decade for boys and girls. 
...
This ALSO looks incredibly janky! I'm going to try to make it a bit more neat by grouping by gender. I'm also only going to keep names that are found in the baby_names list. 
...
## BILLBOARD - musicians
I will see if I get to comparing these! It can be interesting to filter the songs to only those containing a name that appears in the babyname list.
```{r}
# infer gender again
...
I will create a function that gets the top increasing names, and then compare them to the top movie actors/actresses in that decade, as well as the popular movie characters in that decade to find "matches" within decades and then we can see when it was most popular to do so and whether it is still a trend in this decade to name your children after movies/characters. 
 
 function: findmatches -> not working so well
 function: newfindmatches -> will print a message saying what % matches there were! 
